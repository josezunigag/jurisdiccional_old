...
f.each(d.children, function (a, b) { c.appendChild(b) }), d.text && c.appendChild(a.createTextNode(d.text))); return c
  } function z (a, b) { var d = a.header[b].length; var c; a.footer && a.footer[b].length > d && (d = a.footer[b].length); for (var e = 0, g = a.body.length; e < g && !(c = a.body[e][b].toString().length, c > d && (d = c), d > 40); e++);return d > 5 ? d : 5 } function v (a) {
    n === o && (n = w.serializeToString(f.parseXML(p['xl/worksheets/sheet1.xml'])).indexOf('xmlns:r') === -1); f.each(a, function (b, d) {
      if (f.isPlainObject(d))v(d); else {
        if (n) {
          var c = d.childNodes[0]
          var e; var g; var h = []; for (e = c.attributes.length - 1; e >= 0; e--) { g = c.attributes[e].nodeName; var q = c.attributes[e].nodeValue; g.indexOf(':') !== -1 && (h.push({ name: g, value: q }), c.removeAttribute(g)) }e = 0; for (g = h.length; e < g; e++)q = d.createAttribute(h[e].name.replace(':', '_dt_b_namespace_token_')), q.value = h[e].value, c.setAttributeNode(q)
        }c = w.serializeToString(d); n && (c.indexOf('<?xml') === -1 && (c = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' + c), c = c.replace(/_dt_b_namespace_token_/g, ':')); c = c.replace(/<row xmlns="" /g,
          '<row ').replace(/<cols xmlns="">/g, '<cols>'); a[b] = c
      }
    })
  } var i = f.fn.dataTable; var h = { version: '1.0.4-TableTools2',
    clients: {},
    moviePath: '',
    nextId: 1,
    $: function (a) {
      typeof a === 'string' && (a = k.getElementById(a)); a.addClass || (a.hide = function () { this.style.display = 'none' }, a.show = function () { this.style.display = '' }, a.addClass = function (a) { this.removeClass(a); this.className += ' ' + a }, a.removeClass = function (a) { this.className = this.className.replace(RegExp('\\s*' + a + '\\s*'), ' ').replace(/^\s+/, '').replace(/\s+$/, '') }, a.hasClass =
function (a) { return !!this.className.match(RegExp('\\s*' + a + '\\s*')) }); return a
    },
    setMoviePath: function (a) { this.moviePath = a },
    dispatch: function (a, b, d) { (a = this.clients[a]) && a.receiveEvent(b, d) },
    ...
    Client: function (a) { this.handlers = {}; this.id = h.nextId++; this.movieId = 'ZeroClipboard_TableToolsMovie_' + this.id; h.register(this.id, this); a && this.glue(a) } }; h.Client.prototype = { id: 0,
    ready: !1,
    movie: null,
    clipText: '',
    fileName: '',
    ...
    glue: function (a, b) {
      this.domElement = h.$(a); var d = 99; this.domElement.style.zIndex && (d = parseInt(this.domElement.style.zIndex,
        10) + 1); var c = h.getDOMObjectPosition(this.domElement); this.div = k.createElement('div'); var e = this.div.style; e.position = 'absolute'; e.left = '0px'; e.top = '0px'; e.width = c.width + 'px'; e.height = c.height + 'px'; e.zIndex = d; typeof b !== 'undefined' && b !== '' && (this.div.title = b); c.width !== 0 && c.height !== 0 && (this.sized = !0); this.domElement && (this.domElement.appendChild(this.div), this.div.innerHTML = this.getHTML(c.width, c.height).replace(/&/g, '&amp;'))
    },
    positionElement: function () {
      var a = h.getDOMObjectPosition(this.domElement)
      var b = this.div.style; b.position = 'absolute'; b.width = a.width + 'px'; b.height = a.height + 'px'; a.width !== 0 && a.height !== 0 && (this.sized = !0, b = this.div.childNodes[0], b.width = a.width, b.height = a.height)
    },
    getHTML: function (a, b) {
      var d = ''; var c = 'id=' + this.id + '&width=' + a + '&height=' + b; if (navigator.userAgent.match(/MSIE/)) {
        var e = location.href.match(/^https/i) ? 'https://' : 'http://'; var d = d + ('<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="' + e + 'download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=10,0,0,0" width="' +
...
a + '" height="' + b + '" name="' + this.movieId + '" align="middle" allowScriptAccess="always" allowFullScreen="false" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" flashvars="' + c + '" wmode="transparent" />'
    ...
    destroy: function () { var a = this; this.domElement && this.div && (f(this.div).remove(), this.div = this.domElement = null, f.each(h.clients, function (b, d) { d === a && delete h.clients[b] })) },
    reposition: function (a) { a && ((this.domElement = h.$(a)) || this.hide()); if (this.domElement && this.div) { var a = h.getDOMObjectPosition(this.domElement); var b = this.div.style; b.left = '' + a.left + 'px'; b.top = '' + a.top + 'px' } },
    clearText: function () { this.clipText = ''; this.ready && this.movie.clearText() },
    appendText: function (a) { this.clipText += a; this.ready && this.movie.appendText(a) },
    setText: function (a) { this.clipText = a; this.ready && this.movie.setText(a) },
    setFileName: function (a) { this.fileName = a; this.ready && this.movie.setFileName(a) },
    setSheetData: function (a) { this.ready && this.movie.setSheetData(JSON.stringify(a)) },
    setAction: function (a) { this.action = a; this.ready && this.movie.setAction(a) },
    addEventListener: function (a, b) { a = a.toString().toLowerCase().replace(/^on/, ''); this.handlers[a] || (this.handlers[a] = []); this.handlers[a].push(b) },
    setHandCursor: function (a) { this.handCursorEnabled = a; this.ready && this.movie.setHandCursor(a) },
    setCSSEffects: function (a) { this.cssEffects = !!a },
    receiveEvent: function (a, b) {
      var d; var a = a.toString().toLowerCase().replace(/^on/,
        ''); switch (a) {
        ...
        for (var c =
0, e = this.handlers[a].length; c < e; c++) { var g = this.handlers[a][c]; if (typeof g === 'function')g(this, b); else if (typeof g === 'object' && g.length == 2)g[0][g[1]](this, b); else if (typeof g === 'string')j[g](this, b) }
      }
    ...
    b) { b.attr('id'); b.parents('html').length ? a.glue(b[0], '') : setTimeout(function () { x(a, b) }, 500) }; var r = function (a, b) { var d = a.filename === '*' && a.title !== '*' && a.title !== o ? a.title : a.filename; typeof d === 'function' && (d = d()); d.indexOf('*') !== -1 && (d = f.trim(d.replace('*', f('title').text()))); d = d.replace(/[^a-zA-Z0-9_\u00A1-\uFFFF\.,\-_ !\(\)]/g, ''); return b === o || !0 === b ? d + a.extension : d }; var A = function (a) { var b = 'Sheet1'; a.sheetName && (b = a.sheetName.replace(/[\[\]\*\/\\\?\:]/g, '')); return b }; var s = function (a, b) {
    var d = b.match(/[\s\S]{1,8192}/g) ||
[]; a.clearText(); for (var c = 0, e = d.length; c < e; c++)a.appendText(d[c])
  }; var y = function (a, b) {
    for (var d = b.newline ? b.newline : navigator.userAgent.match(/Windows/) ? '\r\n' : '\n', c = a.buttons.exportData(b.exportOptions), e = b.fieldBoundary, g = b.fieldSeparator, f = RegExp(e, 'g'), h = b.escapeChar !== o ? b.escapeChar : '\\', j = function (a) { for (var b = '', c = 0, d = a.length; c < d; c++)c > 0 && (b += g), b += e ? e + ('' + a[c]).replace(f, h + e) + e : a[c]; return b }, k = b.header ? j(c.header) + d : '', m = b.footer && c.footer ? d + j(c.footer) : '', l = [], u = 0, B = c.body.length; u < B; u++)l.push(j(c.body[u]))
    return { str: k + l.join(d) + m, rows: l.length }
  }; var t = { available: function () { return h.hasFlash() }, init: function (a, b, d) { h.moviePath = i.Buttons.swfPath; var c = new h.Client(); c.setHandCursor(!0); c.addEventListener('mouseDown', function () { d._fromFlash = !0; a.button(b[0]).trigger(); d._fromFlash = !1 }); x(c, b); d._flash = c }, destroy: function (a, b, d) { d._flash.destroy() }, fieldSeparator: ',', fieldBoundary: '"', exportOptions: {}, title: '*', filename: '*', extension: '.csv', header: !0, footer: !1 }; try { var w = new XMLSerializer(); var n } catch (C) {} var p =
{ '_rels/.rels': '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/></Relationships>',
  ...
  i.Buttons.swfPath = '//cdn.datatables.net/buttons/1.2.0/swf/flashExport.swf'; i.Api.register('buttons.resize()', function () { f.each(h.clients, function (a, b) { b.domElement !== o && b.domElement.parentNode && b.positionElement() }) }); i.ext.buttons.copyFlash = f.extend({}, t, { className: 'buttons-copy buttons-flash',
    ...
    fieldSeparator: '\t',
    fieldBoundary: '' }); i.ext.buttons.csvFlash = f.extend({}, t, { className: 'buttons-csv buttons-flash', text: function (a) { return a.i18n('buttons.csv', 'CSV') }, action: function (a, b, d, c) { a = c._flash; b = y(b, c); b = c.customize ? c.customize(b.str, c) : b.str; a.setAction('csv'); a.setFileName(r(c)); s(a, b) }, escapeChar: '"' }); i.ext.buttons.excelFlash = f.extend({}, t, { className: 'buttons-excel buttons-flash',
    text: function (a) { return a.i18n('buttons.excel', 'Excel') },
    action: function (a, b, d, c) {
      var a = c._flash; var e = 0; var g = f.parseXML(p['xl/worksheets/sheet1.xml']); var h = g.getElementsByTagName('sheetData')[0]; var d = { _rels: { '.rels': f.parseXML(p['_rels/.rels']) }, xl: { _rels: { 'workbook.xml.rels': f.parseXML(p['xl/_rels/workbook.xml.rels']) }, 'workbook.xml': f.parseXML(p['xl/workbook.xml']), 'styles.xml': f.parseXML(p['xl/styles.xml']), worksheets: { 'sheet1.xml': g } }, '[Content_Types].xml': f.parseXML(p['[Content_Types].xml']) }; var b = b.buttons.exportData(c.exportOptions)
      var j; var k; var i = function (a) {
        j = e + 1; k = l(g, 'row', { attr: { r: j } }); for (var b = 0, c = a.length; b < c; b++) {
          for (var d = b, i = ''; d >= 0;)i = String.fromCharCode(d % 26 + 65) + i, d = Math.floor(d / 26) - 1; d = i + '' + j; if (a[b] === null || a[b] === o)a[b] = ''; typeof a[b] === 'number' || a[b].match && f.trim(a[b]).match(/^-?\d+(\.\d+)?$/) && !f.trim(a[b]).match(/^0\d+/) ? d = l(g, 'c', { attr: { t: 'n', r: d }, children: [l(g, 'v', { text: a[b] })] }) : (i = !a[b].replace ? a[b] : a[b].replace(/[\x00-\x09\x0B\x0C\x0E-\x1F\x7F-\x9F]/g, ''), d = l(g, 'c', { attr: { t: 'inlineStr', r: d },
            children: { row: l(g,
              'is', { children: { row: l(g, 't', { text: i }) } }) } })); k.appendChild(d)
        }h.appendChild(k); e++
    ...
    extension: '.xlsx' }); i.ext.buttons.pdfFlash = f.extend({}, t, { className: 'buttons-pdf buttons-flash',
    text: function (a) { return a.i18n('buttons.pdf', 'PDF') },
    action: function (a, b, d, c) {
      var a = c._flash; var e = b.buttons.exportData(c.exportOptions); var g = b.table().node().offsetWidth; var f = b.columns(c.columns).indexes().map(function (a) { return b.column(a).header().offsetWidth / g }); a.setAction('pdf'); a.setFileName(r(c)); s(a, JSON.stringify({ title: r(c,
        !1),
      message: typeof c.message === 'function' ? c.message(b, d, c) : c.message,
      colWidth: f.toArray(),
