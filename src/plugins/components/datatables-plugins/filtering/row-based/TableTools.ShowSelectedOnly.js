...
(function (window, document, $, undefined) {
  $.fn.dataTable.SelectedLengthMenu = function (oSettings) {
    ...
    if (!oFilterSelectedOptions) { oFilterSelectedOptions = { 'AllText': 'All Items', 'SelectedText': 'Selected Items' } }
    ...
    if (sLengthMenu == 'Show _MENU_ entries') { sLengthMenu = 'Show _MENU_ of _SELECTEDMENU_' }
    ...
    if ($lengthSelect.length == 0) { $lengthSelect = $('select :eq(0)', nLength) }
    ...
    if ($filterSelectedSelect.length == 0) { $filterSelectedSelect = $('select :eq(1)', nLength) }
  ...
  $.fn.dataTableExt.afnFiltering.push(
    function (oSettings, aData, iDataIndex) {
      ...
      if ($filterSelectedSelect.length == 0) { return true } // feature not enabled

      if ($filterSelectedSelect.val() == 'All') { return true } // all items selected
