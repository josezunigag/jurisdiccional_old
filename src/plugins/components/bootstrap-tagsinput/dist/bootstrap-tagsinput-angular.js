angular.module('bootstrap-tagsinput', [])
  .directive('bootstrapTagsinput', [function () {
    function getItemProperty (scope, property) {
      ...
      if (angular.isFunction(scope.$parent[property])) { return scope.$parent[property] }
      ...
      link: function (scope, element, attrs) {
        $(function () {
          ...
          select.on('itemAdded', function (event) {
            if (scope.model.indexOf(event.item) === -1) { scope.model.push(event.item) }
          })
          ...
          scope.$watch('model', function () {
            ...
            var removed = prev.filter(function (i) { return scope.model.indexOf(i) === -1 })
            var i
