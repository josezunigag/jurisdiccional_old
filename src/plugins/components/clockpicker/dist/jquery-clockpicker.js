...
(function () {
  ...
  var svgSupported = 'SVGAngle' in window && (function () {
    var supported
  ...
  function ClockPicker (element, options) {
    ...
    var hoursView = popover.find('.clockpicker-hours')
    var minutesView = popover.find('.clockpicker-minutes')
    var amPmBlock = popover.find('.clockpicker-am-pm-block')
    ...
    if (options.twelvehour) {
      ...
      for (i = 0; i < 24; i += 1) {
        ...
        radian = i / 6 * Math.PI
        var inner = i > 0 && i < 13
        var radius = inner ? innerRadius : outerRadius
        tick.css({
          left: dialRadius + Math.sin(radian) * radius - tickRadius,
          top: dialRadius - Math.cos(radian) * radius - tickRadius
        })
    ...
    function mousedown (e, space) {
      ...
      var x0 = offset.left + dialRadius
      var y0 = offset.top + dialRadius
      var dx = (isTouch ? e.originalEvent.touches[0] : e).pageX - x0
      var dy = (isTouch ? e.originalEvent.touches[0] : e).pageY - y0
      ...
      $doc.off(mousemoveEvent).on(mousemoveEvent, function (e) {
        ...
        var y = (isTouch ? e.originalEvent.touches[0] : e).pageY - y0
        if (!moved && x === dx && y === dy) {
      ...
      $doc.off(mouseupEvent).on(mouseupEvent, function (e) {
        ...
        var y = (isTouch ? e.originalEvent.changedTouches[0] : e).pageY - y0
  ...
  ClockPicker.prototype.setHand = function (x, y, roundBy5, dragging) {
    ...
    var inner = isHours && z < (outerRadius + innerRadius) / 2
    var radius = inner ? innerRadius : outerRadius
    var value
